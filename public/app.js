(() => {
  ...
  var assets_default = [...actorsAssets, ...propsAssets];
  ...
  // src/utils/decodeAnimation.js
  function decodeAnimation_default({ id, response, animationWidth }) {
  ...
  // src/utils/decodeImage.js
  function decodeImage_default({ id, response }) {
    const dataView = new DataView(new Uint8Array(response.data).buffer);
    let offset = dataView.getUint32(10, true);
    const size = dataView.getUint32(14, true);
    const width = dataView.getInt32(18, true);
  ...
  // src/utils/fetchAsset.js
  async function fetchAsset_default({ url, id, container, key, animationWidth }) {
    return axios_default.get(url, { responseType: "arraybuffer" }).then((response) => {
      const decode = animationWidth ? decodeAnimation_default : decodeImage_default;
  ...
  // src/initilizeControls.js
  var initilizeControls = () => {
    window.controls = {};
  ...
  var initilizeControls_default = initilizeControls;

  // src/init.js
  var init = async () => {
    await Promise.all(assets_default.map((asset) => fetchAsset_default(asset)));
  ...
  // src/handlers/handleControls.js
  var validateControl = (c) => c < 0 ? 0 : c;
  ...
  // src/constants.js
  var pixelSize = 1;
  var lightColor = "rgb(252,255,204)";
  var transparentColor = "rgba(0,0,0,0)";

  // src/utils/drawAnimation.js
  window.animations = [];
  var initilizeFrameChange = (frameId, framesCount, frameDelay) => {
    if (!window.animations[frameId]) {
      window.animations[frameId] = {
        currentFrame: 0
      };
      window.animations[frameId].timer = setInterval(() => {
        window.animations[frameId].currentFrame = (window.animations[frameId].currentFrame + 1) % framesCount;
      }, frameDelay);
    }
    return window.animations[frameId].timer;
  };
  function drawAnimation_default({ animations, width, height, id: animationId }, position, { drawId = "", frameDelay = 100 } = {}) {
    const currentFrame = window.animations[animationId + drawId]?.currentFrame || 0;
    for (let py = 0; py < height; py++) {
      for (let px = 0; px < width; px++) {
        context.fillStyle = animations[currentFrame][px + py * width] ? lightColor : transparentColor;
        context.fillRect(
  ...
  function drawImage_default({ pixels, width, height }, position) {
  ...
  var loop = () => {
    ...
    drawAnimation_default(props.helmet, { x: 50, y: 50 }, { frameDelay: 500 });
    drawAnimation_default(
      props.helmet,
      { x: 40, y: 50 },
      { frameDelay: 500, drawId: "1" }
    );
    drawImage_default(props.door, { x: 60, y: 60 });
  ...
  // src/main.js
  var main = async () => {
    await levels_default[currentLevel].initilize();
    levels_default[currentLevel].loop();
  };
